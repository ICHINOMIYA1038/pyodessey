...
import lessonsMeta from "./js-lessons-meta.json";
...
export function getJsLessonSlugs(): string[] {
  return (lessonsMeta as Record<string, unknown>[]).map(
    (l) => l.slug as string
  );
}

export function getAllJsLessons(): LessonMeta[] {
  return (lessonsMeta as Record<string, unknown>[])
    .map((l) => parseMeta(l))
    .sort((a, b) => a.order - b.order);
}

export function getJsLessonBySlug(slug: string): Lesson | null {
  try {
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const data = require(`./js-lessons-content/${slug}.json`) as {
