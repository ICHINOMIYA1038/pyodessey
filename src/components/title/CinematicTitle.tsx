...
function makeLetterVariants(titleLength: number): Variants {
  return {
    hidden: {
...
export function CinematicTitle() {
  const { name, headerTitle } = useAppConfig();
  const letterVariants = makeLetterVariants(name.length);
  return (
